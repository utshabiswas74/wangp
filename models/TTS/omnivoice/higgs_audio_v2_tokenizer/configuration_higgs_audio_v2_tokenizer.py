...
class HiggsAudioV2TokenizerConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.acoustic_model_config = self._resolve_sub_config(
            acoustic_model_config,
            default_model_type="dac",
            default_kwargs=self._default_acoustic_model_config_kwargs,
        )
        self.semantic_model_config = self._resolve_sub_config(
            semantic_model_config,
            default_model_type="hubert",
            default_kwargs=self._default_semantic_model_config_kwargs,
        )
    ...
    @staticmethod
    def _resolve_sub_config(value, default_model_type, default_kwargs):
        if value is None:
            return CONFIG_MAPPING[default_model_type](**default_kwargs)
        if isinstance(value, PretrainedConfig):
            return value
        value = dict(value)
        model_type = value.get("model_type", default_model_type)
        value["model_type"] = model_type
        return CONFIG_MAPPING[model_type](**{**default_kwargs, **value})
