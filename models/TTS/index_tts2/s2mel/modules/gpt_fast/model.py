...
class Transformer(nn.Module):
    ...
    def forward(self,
        ...
        freqs_src = self.freqs_cis
        if freqs_src.device != x.device:
            freqs_src = freqs_src.to(x.device)
        input_pos_for_freq = input_pos if input_pos.device == freqs_src.device else input_pos.to(freqs_src.device)
        if mask is None: # in case of non-causal model
            causal_mask = self.causal_mask
            if causal_mask.device != input_pos_for_freq.device:
                causal_mask = causal_mask.to(input_pos_for_freq.device)
            if not self.training and self.use_kv_cache:
                mask = causal_mask[None, None, input_pos_for_freq]
            else:
                mask = causal_mask[None, None, input_pos_for_freq]
                mask = mask[..., input_pos_for_freq]
        freqs_cis = freqs_src[input_pos_for_freq]
        if context is not None:
            context_pos_for_freq = context_input_pos if context_input_pos.device == freqs_src.device else context_input_pos.to(freqs_src.device)
            context_freqs_cis = freqs_src[context_pos_for_freq]
        else:
