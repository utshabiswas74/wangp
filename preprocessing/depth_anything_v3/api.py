...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        if image.is_floating_point() and model_dtype in (torch.float16, torch.bfloat16):
            image = image.to(dtype=model_dtype)
        use_autocast = image.device.type == "cuda" and model_dtype == torch.float32
        with torch.no_grad():
            with torch.autocast(device_type=image.device.type, dtype=torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16, enabled=use_autocast, cache_enabled=False):
                return self.model(
    ...
    def _prepare_model_inputs(
        ...
        image_dtype = model_dtype if model_dtype in (torch.float16, torch.bfloat16) else torch.float32
        imgs = imgs_cpu.to(device=device, dtype=image_dtype, non_blocking=True)[None]
