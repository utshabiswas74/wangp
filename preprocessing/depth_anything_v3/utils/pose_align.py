...
from ..utils.geometry import affine_inverse, affine_inverse_np
...
def _umeyama_sim3_from_paths(pose_ref, pose_est):
    src = pose_est[:, :3, 3].astype(np.float64)
    dst = pose_ref[:, :3, 3].astype(np.float64)
    src_mean = src.mean(axis=0)
    dst_mean = dst.mean(axis=0)
    src_centered = src - src_mean
    dst_centered = dst - dst_mean
    cov = dst_centered.T @ src_centered / max(src.shape[0], 1)
    ...
    s = float((singular_values * sign).sum() / max(src_var, np.finfo(np.float64).eps))
    t = dst_mean - s * (r @ src_mean)
    r = r.astype(pose_ref.dtype, copy=False)
    t = t.astype(pose_ref.dtype, copy=False)
