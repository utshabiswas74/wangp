...
def load_config(path: str, argv: List[str] = None) -> Union[DictConfig, ListConfig]:
    ...
    Supports both file paths and module paths (e.g., .configs.giant).
    """
...
def import_item(path: str, name: str) -> Any:
    ...
    """
    return getattr(importlib.import_module(path, package=__package__) if path.startswith(".") else importlib.import_module(path), name)
