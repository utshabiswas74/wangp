...
export class CropTool implements Tool {
	...
	private wangp_crop_update_scheduled = false;
	private bound_pointer_move: ((event: FederatedPointerEvent) => void) | null =
		null;
	...
	async setup(
		...
		this.unsubscribers.push(context.dimensions.subscribe((dimensions) => {
			...
			if (this.crop_ui_container) {
				this.wangp_request_crop_update();
			}
	...
	cleanup(): void {
		...
		if (this.crop_ui_container) {
			...
			this.crop_ui_container.destroy({ children: true });
		}
		...
		this.wangp_crop_update_scheduled = false;
	...
	public wangp_reset_interaction(): void {
		...
		this.has_been_manually_changed = false;
		this.wangp_request_crop_update();
	}

	public wangp_cancel_active_drag(): void {
		if (!this.is_dragging && !this.is_dragging_window) return;
		this.is_dragging = false;
		this.is_dragging_window = false;
		this.selected_handle = null;
		this.last_pointer_position = null;
		...
		this.active_corner_index = -1;
		this.active_edge_index = -1;
		if (this.crop_mask) this.crop_mask.visible = true;
		this.wangp_request_crop_update();
	}

	private wangp_request_crop_update(): void {
		if (this.wangp_crop_update_scheduled) return;
		this.wangp_crop_update_scheduled = true;
		requestAnimationFrame(() => {
			this.wangp_crop_update_scheduled = false;
			if (!this.crop_mask || !this.crop_ui_container) return;
			this.update_crop_ui();
			if (!this.is_dragging && !this.is_dragging_window) this.update_crop_mask();
			if (this.crop_mask.visible && this.image_editor_context.background_image) {
				this.image_editor_context.background_image.setMask(this.crop_mask);
			}
			(this.image_editor_context.app as any).wangp_request_render?.();
		});
	}
	...
	private handle_pointer_down(
		...
		if (this.crop_mask) this.crop_mask.visible = false;
	...
	public get_image(): Promise<Blob | null> {
		...
		const source = this.image_editor_context.get_source_image();
		if (source) return this.get_image_from_source(source);
	...
	private async get_image_from_source(source: Blob | File): Promise<Blob | null> {
		const background = this.image_editor_context.background_image;
		if (!background || this.crop_bounds.width <= 0 || this.crop_bounds.height <= 0) return null;

		const bitmap = await createImageBitmap(source);
		const canvas = document.createElement("canvas");
		canvas.width = Math.max(1, Math.round(this.crop_bounds.width));
		canvas.height = Math.max(1, Math.round(this.crop_bounds.height));
		...
		ctx.drawImage(
			bitmap,
			background.x - this.crop_bounds.x,
			background.y - this.crop_bounds.y,
			background.width,
			background.height
		);
		bitmap.close?.();
