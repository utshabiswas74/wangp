...
export class ZoomTool implements Tool {
	...
	private bound_wheel: EventListener | null = null;
	private bound_touch_start: EventListener | null = null;
	private bound_touch_move: EventListener | null = null;
	private bound_touch_end: EventListener | null = null;
	private bound_pointer_down: EventListener | null = null;
	private bound_pointer_move: EventListener | null = null;
	private bound_pointer_up: EventListener | null = null;
	...
	set_zoom(zoom_level: number | "fit"): void {
		this.local_dimensions = get(this.image_editor_context.dimensions);
		this.local_scale = get(this.image_editor_context.scale);
		this.local_position = get(this.image_editor_context.position);
	...
	async setup(
		...
		this.unsubscribers.push(this.image_editor_context.dimensions.subscribe((dimensions) => {
			this.local_dimensions = dimensions;
		}));

		this.unsubscribers.push(this.image_editor_context.scale.subscribe((scale) => {
			this.local_scale = scale;
		}));

		this.unsubscribers.push(this.image_editor_context.position.subscribe((position) => {
	...
	private setup_event_listeners(): void {
		...
		this.bound_wheel = this.handle_wheel.bind(this) as EventListener;
		stage.addEventListener("wheel", this.bound_wheel, { passive: false });

		// Setup touch/pointer events based on device
		if ("ontouchstart" in window) {
			this.bound_touch_start = this.handle_touch_start.bind(this) as EventListener;
			this.bound_touch_move = this.handle_touch_move.bind(this) as EventListener;
			this.bound_touch_end = this.handle_touch_end.bind(this) as EventListener;
			stage.addEventListener("touchstart", this.bound_touch_start);
			stage.addEventListener("touchmove", this.bound_touch_move);
			stage.addEventListener("touchend", this.bound_touch_end);
		} else {
			this.bound_pointer_down = this.handle_pointer_down.bind(this) as EventListener;
			this.bound_pointer_move = this.handle_pointer_move.bind(this) as EventListener;
			this.bound_pointer_up = this.handle_pointer_up.bind(this) as EventListener;
			stage.addEventListener("pointerdown", this.bound_pointer_down);
			stage.addEventListener("pointermove", this.bound_pointer_move);
			stage.addEventListener("pointerup", this.bound_pointer_up);
			stage.addEventListener("pointerupoutside", this.bound_pointer_up);
		}
	}
	...
	private cleanup_event_listeners(): void {
		const app = this?.image_editor_context?.app;
		const stage = app?.stage;
		if (!stage) return;
		app.canvas?.removeEventListener("wheel", this.prevent_default);
		if (this.bound_wheel) {
			stage.removeEventListener("wheel", this.bound_wheel);
			this.bound_wheel = null;
		}
		if (this.bound_touch_start) {
			stage.removeEventListener("touchstart", this.bound_touch_start);
			this.bound_touch_start = null;
		}
		if (this.bound_touch_move) {
			stage.removeEventListener("touchmove", this.bound_touch_move);
			this.bound_touch_move = null;
		}
		if (this.bound_touch_end) {
			stage.removeEventListener("touchend", this.bound_touch_end);
			this.bound_touch_end = null;
		}
		if (this.bound_pointer_down) {
			stage.removeEventListener("pointerdown", this.bound_pointer_down);
			this.bound_pointer_down = null;
		}
		if (this.bound_pointer_move) {
			stage.removeEventListener("pointermove", this.bound_pointer_move);
			this.bound_pointer_move = null;
		}
		if (this.bound_pointer_up) {
			stage.removeEventListener("pointerup", this.bound_pointer_up);
			stage.removeEventListener("pointerupoutside", this.bound_pointer_up);
