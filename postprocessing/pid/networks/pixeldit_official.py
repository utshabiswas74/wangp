...
from postprocessing.pid.utils.context_parallel import cat_outputs_cp_with_grad
...
def apply_adaln(x, shift, scale):
    return x.mul_(scale + 1).add_(shift)
...
def get_pid_linear_split_map(hidden_size: int, pixel_attn_hidden_size: int):
    return {
        "qkv_x": {"mapped_modules": ["q_x", "k_x", "v_x"], "split_sizes": [hidden_size, hidden_size, hidden_size]},
        "qkv": {"mapped_modules": ["q", "k", "v"], "split_sizes": [pixel_attn_hidden_size, pixel_attn_hidden_size, pixel_attn_hidden_size]},
    }
...
class RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        if hidden_states.is_floating_point() and hidden_states.dtype != self.weight.dtype:
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        if self.training or x.ndim != 3:
            return self._forward_inner(x)
        seq_len = x.shape[1]
        chunk_tokens = max(1, min(seq_len, x.numel() // max(x.shape[0] * self.hidden_dim, 1)))
        if chunk_tokens >= seq_len:
            return self._forward_inner(x)
        out = torch.empty_like(x)
        for start in range(0, seq_len, chunk_tokens):
            end = min(start + chunk_tokens, seq_len)
            out[:, start:end].copy_(self._forward_inner(x[:, start:end]))
        return out
...
def precompute_freqs_cis_2d(dim: int, height: int, width: int, theta: float = 10000.0, scale=16.0):
    ...
    freqs = torch.stack([x_freqs, y_freqs], dim=-1).reshape(height * width, -1)
...
def apply_rotary_emb_inplace(x_list: list, freqs_cis: torch.Tensor) -> torch.Tensor:
    ...
    cos = freqs_cis[..., 0].view(1, freqs_cis.shape[0], 1, -1)
    sin = freqs_cis[..., 1].view(1, freqs_cis.shape[0], 1, -1)
    ...
    if x.dtype == torch.float32:
        ...
        x1.mul_(cos).addcmul_(scratch, sin)
    else:
        scratch = x0.float()
        scratch.mul_(sin).addcmul_(x1, cos)
        x0.copy_(x0.float().mul_(cos).addcmul_(x1, sin, value=-1))
        x1.copy_(scratch)
    del scratch
    return x
...
class RotaryAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos, mask) -> torch.Tensor:
        ...
        B, N, C = x.shape
        shape = (B, N, self.num_heads, C // self.num_heads)
        if hasattr(self, "qkv"):
        ...
        if self._cp_group is None:
            q = apply_rotary_emb_inplace([q], pos)
            k = apply_rotary_emb_inplace([k], pos)
        else:
            ...
            q = apply_rotary_emb_inplace([q], pos_local)
        ...
        qkv_list = [q, k, v]
        del q, k, v
        x = pay_attention(qkv_list, attention_mask=mask, recycle_q=True)
...
class MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.training or x.ndim != 3:
            return self._forward_inner(x)
        leading = x.shape[0]
        chunk_leading = max(1, min(leading, x.numel() // max(x.shape[1] * self.hidden_dim, 1)))
        if chunk_leading >= leading:
            return self._forward_inner(x)
        out = torch.empty_like(x)
        for start in range(0, leading, chunk_leading):
            end = min(start + chunk_leading, leading)
            out[start:end].copy_(self._forward_inner(x[start:end]))
        return out
...
class PixelTokenEmbedder(nn.Module):
    ...
    def _fetch_pixel_pos_patch(self, patch_size: int, device, dtype):
        ...
        pos = torch.from_numpy(pos_np)  # [P2, D]
    ...
    def _fetch_pixel_pos_image(self, height: int, width: int, device, dtype):
        if height == width:
            ...
            pos = torch.from_numpy(pos_np)  # [H*W, D]
            self._pos_cache[key] = pos
            return pos.to(device=device, dtype=dtype)
        else:
            key = ("image", height, width)
    ...
    def forward(self, inputs: torch.Tensor, img_height: int = None, img_width: int = None, patch_size: int = None):
        ...
        if inputs.dim() == 3:
            ...
            patch_sz = int(p2**0.5)
            pos = self._fetch_pixel_pos_patch(patch_sz, device, dtype)  # [P2, D]
            x = self.proj(inputs)
            inputs = None
            x.add_(pos.unsqueeze(0))
            del pos
            return x
        elif inputs.dim() == 4:
            # Image mode: [B, C, H, W]
...
class PiTBlock(nn.Module):
    ...
    def _chunk_size_for_pixels(self, BL: int, P2: int) -> int:
        cond_width = 6 * self.pixel_dim
        return max(1, min(BL, (BL * P2 * self.pixel_dim) // max(P2 * cond_width, 1)))
    ...
    def _fetch_pos(self, height: int, width: int, device):
        ...
        if self.rope_mode == "ntk_aware":
            pos = precompute_freqs_cis_2d_ntk(head_dim, height, width, self.rope_ref_grid_h, self.rope_ref_grid_w)
        else:
            pos = precompute_freqs_cis_2d(head_dim, height, width)
        self._pos_cache[key] = pos
    ...
    def forward(
        ...
        B = BL // L_local
        chunk_size = self._chunk_size_for_pixels(BL, P2)
        x_comp_flat = torch.empty((BL, self.attn_dim), device=x.device, dtype=x.dtype)
        for start in range(0, BL, chunk_size):
            end = min(start + chunk_size, BL)
            cond_params = self.adaLN_modulation(s_cond[start:end]).view(end - start, P2, 6 * self.pixel_dim)
            shift_msa, scale_msa, _, _, _, _ = torch.chunk(cond_params, 6, dim=-1)
            x_norm = apply_adaln(self.norm1(x[start:end]), shift_msa, scale_msa)
            x_comp_flat[start:end].copy_(self.compress_to_attn(x_norm.view(end - start, P2 * self.pixel_dim)))
            del cond_params, shift_msa, scale_msa, x_norm
        pos_comp = self._fetch_pos(Hs, Ws, x.device)
        x_list = [x_comp_flat.view(B, L_local, self.attn_dim)]
        x_comp_flat = None
        attn_out = self.attn(x_list, pos_comp, mask)
        attn_flat = attn_out.reshape(BL, self.attn_dim)
        del attn_out
        for start in range(0, BL, chunk_size):
            end = min(start + chunk_size, BL)
            cond_params = self.adaLN_modulation(s_cond[start:end]).view(end - start, P2, 6 * self.pixel_dim)
            _, _, gate_msa, shift_mlp, scale_mlp, gate_mlp = torch.chunk(cond_params, 6, dim=-1)
            attn_exp = self.expand_from_attn(attn_flat[start:end]).view(end - start, P2, self.pixel_dim)
            x_chunk = x[start:end]
            x_chunk.add_(attn_exp.mul_(gate_msa))
            del attn_exp, gate_msa
            mlp_out = self.mlp(apply_adaln(self.norm2(x_chunk), shift_mlp, scale_mlp))
            x_chunk.add_(mlp_out.mul_(gate_mlp))
            del cond_params, shift_mlp, scale_mlp, gate_mlp, mlp_out
        del attn_flat
        return x
...
class MMDiTJointAttention(nn.Module):
    ...
    def forward(
        ...
        # QKV for image
        x_shape = (B, Nx, self.num_heads, C // self.num_heads)
        if hasattr(self, "qkv_x"):
            ...
            qx = self.q_x(x).view(x_shape)
            kx = self.k_x(x).view(x_shape)
            vx = self.v_x(x).view(x_shape)
        x = None
        qx = self.q_norm_x(qx)
        kx = self.k_norm_x(kx)
        vx = vx.contiguous()
        ...
        if self._cp_group is None:
            qx = apply_rotary_emb_inplace([qx], pos_img)
            kx = apply_rotary_emb_inplace([kx], pos_img)
        else:
            cp_size = self._cp_group.size()
            cp_rank = self._cp_group.rank()
            ...
            qx = apply_rotary_emb_inplace([qx], pos_img_local)
            # `all_gather` requires contiguous tensors; the qkv permute leaves k/v as non-contiguous views.
        ...
        if pos_txt is not None:
            qy = apply_rotary_emb_inplace([qy], pos_txt)
            ky = apply_rotary_emb_inplace([ky], pos_txt)

        # Shared attention expects [B, S, H, Hc]; build joint sequence [text, image].
        # Under CP: qx is [B, Nx_local, H, Hc]; kx, vx are [B, Nx_full, H, Hc].
        q_joint = qy.new_empty((B, Ny + qx.shape[1], self.num_heads, self.head_dim))
        q_joint[:, :Ny].copy_(qy)
        q_joint[:, Ny:].copy_(qx)
        del qy, qx
        k_joint = ky.new_empty((B, Ny + kx.shape[1], self.num_heads, self.head_dim))
        k_joint[:, :Ny].copy_(ky)
        k_joint[:, Ny:].copy_(kx)
        del ky, kx
        v_joint = vy.new_empty((B, Ny + vx.shape[1], self.num_heads, self.head_dim))
        v_joint[:, :Ny].copy_(vy)
        v_joint[:, Ny:].copy_(vx)
        del vy, vx

        qkv_list = [q_joint, k_joint, v_joint]
        del q_joint, k_joint, v_joint
        out_joint = pay_attention(qkv_list, attention_mask=attn_mask, recycle_q=True)
        ...
        if self.training:
            out_x = self.proj_drop_x(out_x)
            out_y = self.proj_drop_y(out_y)
        return out_x, out_y
...
class MMDiTBlockT2I(nn.Module):
    ...
    def forward(self, x, y, c, pos_img, pos_txt=None, attn_mask=None):
        ...
        y_list = [y_norm]
        del x_norm, y_norm
        attn_x, attn_y = self.attn(x_list, y_list, pos_img, pos_txt, attn_mask)
        x.add_(attn_x.mul_(gate_msa_x))
        y.add_(attn_y.mul_(gate_msa_y))
        del attn_x, attn_y

        # 2) Per-stream MLP with AdaLN
        mlp_x = self.mlp_x(apply_adaln(self.norm_x2(x), shift_mlp_x, scale_mlp_x))
        mlp_y = self.mlp_y(apply_adaln(self.norm_y2(y), shift_mlp_y, scale_mlp_y))
        x.add_(mlp_x.mul_(gate_mlp_x))
        y.add_(mlp_y.mul_(gate_mlp_y))
        del mlp_x, mlp_y
        return x, y
...
class _TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if use_ts:
            ...
            x.add_(attn_out.mul_(gate_msa))
            del attn_out
        else:
            x_norm = apply_adaln(self.norm1(x), shift_msa, scale_msa)
            x_list = [x_norm]
            del x_norm
            attn_out = self.attn(x_list, pos, mask)
            x.add_(attn_out.mul_(gate_msa))
            del attn_out
        mlp_out = self.mlp(apply_adaln(self.norm2(x), shift_mlp, scale_mlp))
        x.add_(mlp_out.mul_(gate_mlp))
        del mlp_out
        return x
    ...
    def forward(self, x: torch.Tensor, height: int, width: int, c: torch.Tensor):
        ...
        for stage in self.stages:
            for blk in stage["blocks"]:
                ...
                x_list = [x]
                x = None
                x = blk(x_list, c, pos, None, H, W) if self.use_attn_token_shuffle else blk(x_list, c, pos, None)
...
class PixDiT_T2I(nn.Module):
    ...
    def fetch_pos_text(self, length, device):
        ...
        head_dim = self.hidden_size // self.num_groups
        # Create real cos/sin frequencies: [length, head_dim//2, 2]
        freqs = 1.0 / (self.text_rope_theta ** (torch.arange(0, head_dim, 2).float() / head_dim))
        positions = torch.arange(0, length).float().unsqueeze(1)  # [length,1]
        angles = positions * freqs.unsqueeze(0)  # [length, head_dim//2]
        freqs_cis = torch.stack([angles.cos(), angles.sin()], dim=-1)
